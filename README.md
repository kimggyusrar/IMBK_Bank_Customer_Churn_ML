...
##  Project Conclusion
본 프로젝트는 **AutoML → Optuna Tuning → Stacking**으로 이어지는 고도화된 모델링 프로세스를 통해 예측 성능을 확보했으며, **SHAP 분석**을 통해 데이터 기반의 구체적인 비즈니스 액션 아이템(고연령층 케어, 고액 자산가 리텐션 등)을 도출하는 성과를 거두었습니다.
...
단순히 예측 성능(F1-Score)을 높이는 것에 그치지 않고, **이탈의 핵심 원인을 데이터로 증명**하고 이를 해결하기 위한 **구체적인 비즈니스 액션 아이템을 도출**했다는 점에서 큰 의의가 있습니다.
1. a
